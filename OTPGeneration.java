...
public class OTPGeneration {
	public static void main(String args[]){
		...
		if(ac_no.length()==11){
			...
			otp+=acc_type.substring(0,2);
			otp+=ac_no.substring(0,1);
			int n=Integer.parseInt(ac_no.substring(0,5));
			int r=0;
			while(n>0 || sum>9){
				if(n==0){
					n=sum;
			...
			otp+=sum;
			otp+=acc_name.substring(0,1);
			System.out.println(otp);
			int n1=Integer.parseInt(ac_no.substring(5,10));
			int r1=0;
			while(n1>0 || sum1>9){
				if(n1==0){
					n1=sum1;
			...
			otp+=sum1;
			otp+=acc_name.substring(acc_name.length()-1);
			otp+=ac_no.substring(ac_no.length()-1);
			System.out.println(otp);
			}
	}
